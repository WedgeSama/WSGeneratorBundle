...
EOT
	...
	protected function execute(InputInterface $input, OutputInterface $output) {
		...
		$generator->generate($input->getOption('namespace'), 
							$input->getOption('bundle'), 
							$input->getOption('dir'), $input->getOption('form'), 
		...
		try {
			$routing->addResource($input->getOption('bundle'), 'yml', 
								$input->getOption('route_prefix'));
		...
		$kernel = new KernelManipulator($this->getContainer()
			->get('kernel'));
	...
	protected function initialize(InputInterface $input, OutputInterface $output) {
		$this->getHelperSet()
			->set(new BundleDialogHelper());
	}

	protected function interact(InputInterface $input, OutputInterface $output) {
		$dialog = $this->getHelperSet()
			->get('ws_bundle_dialog_helper');
		...
		$namespace = $dialog->askNamespace($output, 
												$input->getOption('namespace'));
		...
		// route
		$route = $dialog->askRoutePerfix($output, 
												$input->getOption(
																'route_prefix'));
		
		// formulaire ?
		$form = $dialog->askConfirmation($output, 
